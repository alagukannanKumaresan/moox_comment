...
class Pi3Controller extends \TYPO3\CMS\Extbase\Mvc\Controller\ActionController {	
    ...
    public function initializeAction() {					
		...
		if($this->settings['ratingRequired']){
			$this->fields['rating']['config']['required'] = 1;
			$this->fields['rating']['config']['validate'] = 1;			
			$this->fields['rating']['config']['data']['data-required'] = 1;
	...
	public function addAction($add = NULL, $ajax = FALSE, $settings = NULL) {
		...
		// no errors -> add review
		if(!count($errors)){
		
			// set item values
			$item = $this->objectManager->get('DCNGmbH\MooxComment\Domain\Model\Review');					
			...
			$item->setRating(trim($add['rating']));
			...
			$this->reviewRepository->add($item);
		...
		if($uid>0){
			...
			if(is_object($item)){
				...
				if($hash==$item->getHash()){
					...
					$this->reviewRepository->update($item);
					$this->persistenceManager->persistAll();
					...
					if(!$ajax){
						...
						$this->redirectToUri($item->getUrlForeign()."#reviewform");
